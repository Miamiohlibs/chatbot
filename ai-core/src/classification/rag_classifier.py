...
class RAGQuestionClassifier:
        ...
        if not exists:
            ...
            self.client.collections.create(
                ...
                properties=[
                    ...
                    wvc.Property(name="is_in_scope", data_type=wvc.DataType.BOOL, description="Whether this is an in-scope example"),
                    wvc.Property(name="description", data_type=wvc.DataType.TEXT, description="Category description"),
                    wvc.Property(name="agent", data_type=wvc.DataType.TEXT, description="Agent to handle this category"),
                    wvc.Property(name="keywords", data_type=wvc.DataType.TEXT_ARRAY, description="Keywords for hybrid search"),
                ]
            )
    ...
    async def initialize_vector_store(self, force_refresh: bool = False):
        ...
        # V4 API: Check if collection already has data
        try:
            collection = self.client.collections.get(self.collection_name)
            result = collection.query.fetch_objects(limit=1)
            if result and len(result.objects) > 0 and not force_refresh:
                return
        except:
            pass
    ...
    async def classify_question(
        ...
        for obj in response.objects:
            # V4 API: distance 0 = perfect match, higher = less similar
            # Convert to certainty (0-1, where 1 = perfect match)
            distance = obj.metadata.distance if obj.metadata else 1.0
            certainty = 1.0 - min(distance, 1.0)  # Cap at 1.0 for safety
            
            matches.append({
                "category": obj.properties.get("category", ""),
                "question": obj.properties.get("question", ""),
                "is_in_scope": obj.properties.get("is_in_scope", True),
        ...
        if not matches:
            if logger:
                logger.log(
                    "⚠️ [RAG Classifier] No matches found, defaulting to general_question"
                )
            return {
        ...
        if top_matches:
            ...
            if not top_match["is_in_scope"]:
                ...
                if logger:
                    logger.log(
                        f"🚫 [RAG Classifier] Negative example match blocked routing - category: {negative_example_category} (confidence: {float(confidence):.2f})"
                    )
        ...
        is_out_of_scope = (top_category == "out_of_scope") or top_category.startswith(
            "out_of_scope_"
        )
        ...
        if is_out_of_scope:
            if logger:
                logger.log(
                    f"🚫 [RAG Classifier] Out-of-scope category detected: {top_category} - skipping clarification"
                )
            return {
        ...
        if len(sorted_categories) > 1:
            ...
            if logger:
                logger.log(
                    f"📊 [RAG Classifier] Top-1: {top_category} ({top_score_float:.3f}) | Top-2: {second_category} ({second_score_float:.3f}) | Margin: {margin:.3f}"
                )
            ...
            if use_llm_fallback and margin < margin_threshold:
                if logger:
                    logger.log(
                        f"🤖 [RAG Classifier] Low margin ({margin:.3f} < {margin_threshold}) - using LLM fallback"
                    )

                llm_result = await self._llm_classify(
                    user_question, sorted_categories[:2], top_matches, logger
                )
        ...
        if len(sorted_categories) > 1:
            ...
            if score_diff < 0.3:
                if logger:
                    logger.log(
                        f"⚠️ [RAG Classifier] Ambiguous: {top_category} ({float(top_score):.2f}) vs {sorted_categories[1][0]} ({float(second_score):.2f})"
                    )
        ...
        if boundary_match:
            ...
            clarification_data = await self._generate_clarification_choices(
                user_question, boundary_match["possible_categories"], top_matches
            )
        ...
        if logger:
            ...
            logger.log(
                f"✅ [RAG Classifier] Classified as: {top_category} (confidence: {float(confidence):.2f}, margin: {margin_str})"
            )
    ...
    async def _llm_classify(
        ...
        examples1 = [
            ex["question"] for ex in similar_examples if ex["category"] == category1
        ][:3]
        examples2 = [
            ex["question"] for ex in similar_examples if ex["category"] == category2
        ][:3]

        examples1_text = (
            "\n".join([f"  - {ex}" for ex in examples1])
            if examples1
            else "  (no examples)"
        )
        examples2_text = (
            "\n".join([f"  - {ex}" for ex in examples2])
            if examples2
            else "  (no examples)"
        )
...
Be decisive. Choose the category that best matches the user's intent."""
        ...
        messages = [
            SystemMessage(
                content="You are an expert question classifier for a university library system."
            ),
        ...
        try:
            ...
            if logger:
                logger.log(
                    f"🤖 [LLM Classifier] Chose: {result['category']} (confidence: {float(result['confidence']):.2f})"
                )
            ...
            if logger:
                logger.log(
                    f"⚠️ [LLM Classifier] Error: {str(e)} - defaulting to top category"
                )
            return {
                "category": category1,
                "confidence": 0.7,
    ...
    async def _generate_clarification_choices(
        ...
        for i, cat in enumerate(
            ambiguous_categories[:3]
        ):  # Max 3 choices + "None of the above"
            desc = get_category_description(cat)
            examples = [
                ex["question"] for ex in similar_examples if ex["category"] == cat
            ][:2]
...
async def classify_with_rag(
    user_question: str, conversation_history: Optional[List[Dict]] = None, logger=None
) -> Dict[str, Any]:
    """
    ...
    return await _classifier_instance.classify_question(
        user_question, conversation_history, logger
    )
