...
**只用 GPT-5.6。** 2026 年 9 月 1 日的决定:5.6 以下的型号一律不留 —— 不做
档位、不做兜底、也不留在费率表里。

| 角色 | 模型 | 每 1M token(输入/缓存/输出) |
|---|---|---|
| *(未启用,但已计价)* | `gpt-5.6-sol` | 4.00 / 0.40 / 20.00 |
| 推理 | `gpt-5.6-terra` | 2.00 / 0.20 / 12.00 |
| 基础 + 省钱 | `gpt-5.6-luna` | 0.20 / 0.02 / 1.20 |
| 向量 | `text-embedding-3-large` | 0.13 —— 只计输入 |

费率为 2026 年 9 月 1 日查得。**sol 是这条线的顶配,在 terra 之上**,所以我们的
推理档用的是中间那个而不是最好的那个。它刻意"只计价、不启用":费率写进表里,
是为了将来切过去时不会突然按 $0 计费。sol 在 8 月 21 日**降了价**(原
5.00 / 0.50 / 30.00),OpenAI 说这是促销价,至少持续到 11 月 21 日。
